...
const Menu: typeof DropdownMenu.Root = DropdownMenu.Root;
const Trigger: typeof DropdownMenu.Trigger = DropdownMenu.Trigger;
...
const Content = ({ className, ...props }: IMenuContent) => (
    <DropdownMenu.Portal>
        <DropdownMenu.Content
            side="bottom"
            align="start"
            className={cn('bg-white border-2 shadow-md absolute top-2 min-w-20', className)}
            {...props}
        />
    </DropdownMenu.Portal>
);
...
const MenuComponent: MenuComponentType = Object.assign(Menu, {
    Trigger,
    Content,
    Item: MenuItem,
});
